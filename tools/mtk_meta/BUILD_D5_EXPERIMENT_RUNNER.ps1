...
'@
  ...
  [int]$ProbeTimeoutSeconds = 60,
'@
  $paramAnchor = $paramAnchor -replace "`r`n", "`n"
  $paramReplacement = $paramReplacement -replace "`r`n", "`n"
...
'@
  ...
  $source = $source.Replace($exportAnchor, $exportReplacement.TrimEnd())
...
'@
  $requestAnchor = $requestAnchor -replace "`r`n", "`n"
  $requestReplacement = $requestReplacement -replace "`r`n", "`n"
